...
function annotate_models_for_rag() {
    ...
    ldraw_annotate_models.py -d "$LDRAW_INFO_DB" -g "$specs_dir/ldraw.lark" -f "$input_dir" -o "$output_dir"
...
function build_db() {
    ...
    cp "$tsvs_dir/COLORS.tsv" .
    cp "$tsvs_dir/MODEL_SIZES.tsv" .
    cp "$tsvs_dir/MODEL_NUM_PARTS.tsv" .
    cp "$tsvs_dir/MODEL_SUBMODELS.tsv" .
    cp "$tsvs_dir/MODEL_AI_CAT_DESC_KWS.tsv" .
    cp "$tsvs_dir/MODEL_AI_CATS_UNIQ.tsv" .
    cp "$tsvs_dir/MODEL_AI_KWS_UNIQ.tsv" .
    cp "$tsvs_dir/PART_BBOXES.tsv" .
    cp "$tsvs_dir/PART_CATEGORIES.tsv" .
    cp "$tsvs_dir/PART_INFOS.tsv" .
    cp "$tsvs_dir/PART_KEYWORDS.tsv" .
...
next_step "Validate models" && \
    ldraw_validator.py -d "$LDRAW_INFO_DB" -g "$specs_dir/ldraw.lark" -f "$tmp_models_dir" > "$S06_OUT"
    # ldraw_validator.py -d "$LDRAW_INFO_DB" -f "$tmp_models_dir" > "$S06_OUT" 2>/dev/null
...
#     ldraw_sanitize.py --filepath "$tmp_rag_models_dir" && \
#     ldraw_sanitize.py --dos2unix "$tmp_rag_models_dir"
...
next_step "Building database" && \
    build_db "$tmp_rag_models_dir" "$tmp_db_dir" "$PREPARE_RAG_DB"
